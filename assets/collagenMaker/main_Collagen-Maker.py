...
COLLAGE_SETTINGS = {
    ...
    'write_letters': False,  # Control letter writing
    'enhance_colors': False  # Control color enhancement
}
...
def adjust_image(image_path):
    ...
    if COLLAGE_SETTINGS['enhance_colors']:
...
def create_collage(images, save_path):
    ...
    for variant_num in range(5):
        ...
        # Overlay STARDUST pattern if enabled
        if COLLAGE_SETTINGS['write_letters']:
            word = "STARDUST"
...
def main():
    image_folder = os.path.join(os.getcwd(),"./assets/collagenmaker")
